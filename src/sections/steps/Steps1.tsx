...
const Steps1: React.FC<Steps1Props> = ({
  title,
  subtitle,
  steps,
  customData
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          {/* Connecting Line */}
          <div 
            className="absolute left-1/2 transform -translate-x-1/2 top-0 bottom-0 w-1 hidden md:block"
          ...
          <div className="space-y-12 md:space-y-0 md:grid md:grid-cols-2 md:gap-x-12 md:gap-y-12">
            {steps.map((step, index) => (
              ...
              >
                {/* Step Number */}
                <div 
                  className={`absolute top-0 ${index % 2 === 0 ? 'md:right-0 md:-mr-6' : 'md:left-0 md:-ml-6'} w-12 h-12 rounded-full flex items-center justify-center z-10 text-white font-bold text-lg`}
                  style={{ 
                ...
                <div 
                  className={`pl-16 ${index % 2 === 0 ? 'md:pl-0 md:pr-16' : ''}`}
                >
                  <h3 
